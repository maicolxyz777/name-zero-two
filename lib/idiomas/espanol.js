...
const smsAvisoMG = () => { return `╰⊱❗️⊱ *عمل سيء للاستخدام* ⊱❗️⊱╮\n\n` }
const smsAvisoEEG = () => { return `╰⊱📩⊱ *تقرير* ⊱📩⊱╮\n\n` }
const smsAvisoEG = () => { return `╰⊱💚⊱ *نجاح* ⊱💚⊱╮\n\n` }

//PARÁMETROS EN COMANDOS
const smsRowner = () => { return `\`\`\`¡¡هذا الأمر فقط أنا كمنشئ الروبوت يمكنه استخدامه!!\`\`\`` }//عدد البوت
...
const smsUnreg = () => { return `\`\`\`¡¡يجب أن تكون مسجلاً اكتب (finalizar) لاستخدام هذا الأوامر كامله للبوت, شكرأ لستخدامك بوت لوسيفار يرجئ التسجيل قبل عرض الاوامر كامله قم بلتسجيل كل الامر المعروض في القوسين ثم اكتب .الاوامر!!\`\`\`` }//للتاكد من تسجيلك اكتب .الاوامر
